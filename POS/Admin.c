...
int Order(int t, char* string)//상품 주문
{
    ...
    {
        char name[16];
        strcpy(name, all_product.list[i].name);
        remove_all_spaces(name); //공백제거
        if (!strcmp(string, name))//메뉴에 상품명이 있으면
        {
            int flag = 0; //테이블 내에 상품이 있는지 확인 .. 있으면 1, 없으면 0
            for (int j = 0; j < tables[t].list_size; j++)
            {
                char tname[16];
                strcpy(tname,tables[t].list[j].name);
                remove_all_spaces(tname); //공백제거
                if (!strcmp(string, tname)) // 테이블 내에 상품이 이미 있음
            ...
            if (!flag) // 테이블 내에 상품이 없음 , malloc 으로 배열 할당 후 추가 주문마다 realloc
            {
                if (tables[t].list_size == 0) {  //주문내역이 아예 비어있음
                    if ((tables[t].list = (Product*)malloc(sizeof(Product))) == NULL)
                        {   //메모리 부족으로 malloc 호출 실패하면 종료됨
                            fprintf(stderr, "메모리가 부족하여 주문이 불가합니다.");
                            exit(EXIT_FAILURE);
                         }
                    strcpy(tables[t].list[0].name, all_product.list[i].name); //상품명 저장
                    tables[t].list[0].price = all_product.list[i].price; //가격 저장
                    tables[t].list[0].sales = 1; //개수 저장
                    tables[t].list_size++; //다음 인덱스를 가리킴
                    }
                else{
                    if ((tables[t].list = realloc(tables[t].list, ((tables[t].list_size + 1) * (sizeof(Product))))) == NULL)
                        {
                            fprintf(stderr, "메모리가 부족하여 주문이 불가합니다.");
                            exit(EXIT_FAILURE);
                         }
...
int Cancle(int t, char* string)//상품 취소
{
    ...
    {
        char tname[16];
        strcpy(tname, tables[t].list[i].name);
        remove_all_spaces(tname); //공백제거
        if (!strcmp(string, tname)) // 주문내역에 상품이 이미 있음
        {	
            if (tables[t].list[i].sales == 0)
            {
                ...
                tables[t].list[i].sales--; //주문한 상품의 개수 -1
                return 1;
            }
        }
        else { //주문내역에 상품명이 없으면
